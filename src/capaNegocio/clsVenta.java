...
public class clsVenta {
    ...
    public ArrayList ventas() throws Exception
    {
        int rpta = 0;
        ArrayList datos = new ArrayList();
        objConectar.conectar();
        Connection con = objConectar.getCon();
        CallableStatement sentencia = con.prepareCall("select * from f_ventasEst()");
        ResultSet resultado = sentencia.executeQuery();
        if (resultado.next())
        {
            datos.add(resultado.getInt("tot"));
            datos.add(resultado.getDouble("suma"));
            datos.add(resultado.getDouble("min"));
            datos.add(resultado.getDouble("max"));
        }
        return datos;
    }
    
    //sentencia.setString(1, nombre);
     public ResultSet listaDetalleVentas(int num) throws Exception
    {
        objConectar.conectar();
        Connection con = objConectar.getCon();
        CallableStatement sentencia = con.prepareCall("select * from  f_ventasDetalle(?) ");
